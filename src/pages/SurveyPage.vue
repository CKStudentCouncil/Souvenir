<template>
  ...
  <div v-else class="survey-admin-page">
    ...
    <section id="overview" class="section-group">
      <div class="stats">
        <div class="stat-card">
      ...
      <!-- 待關注項目：自動抓出分數最低的題目，優先看這裡 -->
      <div v-if="attentionQuestions.length" class="panel callout">
        <h2>待關注項目</h2>
        <p class="callout-sub">分數最低的三題，建議優先檢視</p>
        <div class="attention-list">
          <div v-for="q in attentionQuestions" :key="q.id" class="attention-row">
            <div class="attention-meta">
              <span class="attention-section">{{ q.sectionTitle }}</span>
              <span class="attention-avg num" :class="tierClass(q.average)">{{ q.average }}</span>
            </div>
            <p class="attention-text">{{ q.text }}</p>
          </div>
        </div>
      </div>
    ...
    <section id="question-detail" class="section-group">
      <p class="group-eyebrow">細部數據</p>

      <!-- 每題詳細數據：逐題平均分數與 1~5 分分布 -->
      <div class="panel">
    ...
    <section id="satisfaction" class="section-group">
      <p class="group-eyebrow">滿意度與趨勢</p>
    ...
    <section id="demographics" class="section-group">
      <p class="group-eyebrow">使用者輪廓</p>
    ...
    <section id="text-feedback" class="section-group">
      <p class="group-eyebrow">文字回饋</p>
...
// 分數分級，用來替平均分數上色：<3.5 需留意、3.5~4.2 普通、>=4.2 良好
...
// 甜甜圈圖：r=40 的圓周長，用 stroke-dasharray/offset 畫出已完成的弧段
...
// 使用者輪廓分頁：把身分/管道/裝置合併成一個 tab 面板，減少頁面長度
const demoTabs = [
...
// 各構面 1~5 分次數分布（把該構面底下所有題目的分數混在一起算）
...
// 待關注項目：所有題目依平均分數排序，取最低的三題（需至少有人作答）
...
// 每日回覆趨勢：把 createdAt 依日期分組計數，畫成折線圖
